...
interface TransformObjectOption extends fabric.IObjectOptions{

}

interface TransformObject {
    original?: TransformObject;
    options?: TransformObjectOption;
    target?: Snap.Element;
    action?: any;
    scaleX?: number;
    scaleY?: number;
...
class Canvas {
    ...
    _currentTransform: TransformObject;
    height: number;
    width: number;
    ...
    getActionFromCorner (target, corner) {
        var action = 'drag';
        if (corner) {
     ...
     */
    setupCurrentTransform (dx, dy, x, y, target: Snap.Element, corner, options) {

        if (!target) {
            return;
        }
        var action = this.getActionFromCorner(target, corner),
            origin = this._getOriginFromCorner(target, corner),
            coords = target.getCoords(),
            scaleX = coords.scaleX,
            scaleY = coords.scaleY,
            left = coords.transformX,
            top = coords.transformY;
        ...
        this._currentTransform = {
            ...
            width: parseFloat(target.attr('initialwidth')) * scaleX, //TODO récupérer la largeur plus proprement
            height: parseFloat(target.attr('initialheight')) * scaleY, //TODO récupérer la hauteur plus proprement
            mouseXSign: 1,
            mouseYSign: 1
        };
    ...
    _scaleObject (x, y, by) {
        var t = this._currentTransform,
            target = t.target,
            lockScalingX = t.options.lockScalingX,
            lockScalingY = t.options.lockScalingY,
            lockScalingFlip = t.options.lockScalingFlip;
     ...
     * @private
     */
    _setObjectScale (localMouse, transform, lockScalingX, lockScalingY, by, lockScalingFlip) {
        var target = transform.target, forbidScalingX = false, forbidScalingY = false,
            strokeWidth = target.stroke ? target.strokeWidth : 0, options = transform.options;
     ...
     * @private
     */
    _performTransformAction (transform, pointer) {
        var x = pointer.x,
            y = pointer.y,
     ...
     * @private
     */
    _scaleObjectEqually (localMouse, target, transform) {
